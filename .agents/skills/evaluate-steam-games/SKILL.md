...
- When `price_status` is `unavailable`, preserve `reason`, mark the current price, regular price, and discount unavailable, and continue. When exactly one ITAD identity is resolved, a current-overview failure does not suppress a valid Steam low or Steam history result. For `itad_rate_limited`, honor `retry_after` when practical and retry once. Treat other ITAD failures as non-fatal.
- If the key is missing, explain that local `itad_api_key` configuration enables pricing and bundle context.
- Preserve `steam_low_reason`, `steam_low_message`, and `steam_low_retry_after` when the Steam low is unavailable. Preserve `steam_low_comparison_reason` and `steam_low_comparison_message` when a valid low cannot be compared with the current price. Use `current_price_unavailable` when a Steam low exists but no current price is available for comparison.
- After shared product lookup and unambiguous identity resolution, current price, Steam store low, Steam-low comparison, Steam history, and bundles fail independently; a failure in one does not suppress valid evidence from the others. An ITAD rate limit or inability to select one identity is a shared prerequisite failure and may suppress dependent standalone-price signals.

The script resolves the Steam app product and associated Steam package products in one ITAD lookup. For standalone pricing, it requires ITAD's current Steam offer to match the regional Steam AppDetails currency plus initial and final minor-unit amounts. Prefer the matching app identity. Only when it does not match, accept a base-price package from Steam's purchase options; accept multiple matching packages only when they map to one ITAD identity, and otherwise mark standalone pricing ambiguous. Never choose by ITAD response order or title. Query store low and history for the exactly matched identity, or for the sole distinct resolved identity when the current overview is unavailable. When multiple identities remain and exact matching cannot select one, mark store low and history unavailable rather than guessing. Query bundle history for every distinct resolved identity and deduplicate bundles by ITAD bundle ID.
...
| **Sustained list-price change** | ↑ Increased from ... to ... on date (+...%) / ↓ Decreased from ... to ... on date (−...%) / None detected / Ambiguous / Insufficient / Unavailable |
...
* Preserve the exact price inputs and historical-low semantics from the pricing workflow. The buyer-facing table labels `Now`, `Regular price`, `Discount`, `Steam recorded low`, and `Compared with recorded low` map respectively to `current_price`, `regular_price`, `discount_percent`, `historical_low_price` with `steam_low_timestamp`, and the required current-versus-historical-low comparison. `Exact-low recurrence` maps to `exact_low_pattern`: map the explicit `insufficient` value to `Insufficient` and null to `Unavailable`. `Recurring realistic sale level` maps to `recurring_sale_price`: map null to `None found` only when Steam history and regular price are both available, and otherwise to `Unavailable`. `Sustained list-price change` maps to `list_price_change`: map its explicit types to their matching states and null to `Unavailable`.
